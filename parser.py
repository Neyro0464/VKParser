...
from DBconn import connection, DB_Insert, disconnect, DB_Insert_group
def parse(DOMAIN_LIST=['incident_nsk'], PERIOD=5, CITY='Новосибирск'):
    ...
    def parse_groups():
        for domain in DOMAIN_LIST:
            ...
            DB_Insert_group( group_info[0]['id'], group_info[0]['name'], f"https://vk.com/{group_info[0]['screen_name']}", CITY)


    #добавить запись в файл/ворд

    # Для каждого поста выводим комментарии
    def parse_wall(conn, offset):
                        ...
                        if wordFilt: #Если список ключевых слов не пуст, то ищем каждое ключевое слово в тексте КАЖДОГО поста
                            for key in wordFilt.keys():
                                for word in wordFilt[key]:
                                    ...
                                    if word in text:
                                        counter_dict[key] += 1
                                        comments = get_comments(owner_id, post['id'])
                                        ...
                                        DB_Insert(conn, post['id'], key, datetime.fromtimestamp(post['date']), 
                                                group_info[0]['id'], addres, word, comments)
